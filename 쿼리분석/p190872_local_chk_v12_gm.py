...
# -*- coding: utf-8 -*-
# ===============================================================
# p190872_local_chk_v12_gm.py (2026-08-11 작성 및 수정)
# [수정 전 백업 파일: p190872_local_chk_v11_gm.py]
# 
# [수정 사항 요약]
#   - 2026-08-11 compare_col1, compare_col2 키값 분리 보완:
#     * compare_col1과 compare_col2의 키값 컨버전 시, 라인 내 default.encrypt/decrypt
#       키 보완 로직이 matched_line 전체 키를 공통 적용하여 타 컬럼 키로 오염되던 현상 수정.
#     * extract_line_enc_key_for_col 함수를 추가하여 각 컬럼(c1, c2) 자신에게 직접
#       적용된 default.encrypt/decrypt 구문만 독립적으로 탐색 및 키 보완 수행.
# ===============================================================
...
# ============================================================
# 특정 컬럼에 직접 적용된 default.encrypt / default.decrypt 키 인자 추출 함수
# ============================================================
def extract_line_enc_key_for_col(col_lower, orig_col, line_text):
    if not col_lower or not line_text:
        return ""
    
    pattern = r"(?i)default\.(?:encrypt|decrypt)\s*\(\s*(.*?)\s*\)"
    for m in re.finditer(pattern, line_text):
        args_str = m.group(1)
        parts = [p.strip() for p in args_str.split(",")]
        if len(parts) >= 2:
            first_args_part = ",".join(parts[:-1])
            key_part = parts[-1].strip("'\" ")
            
            if re.search(r"\b%s\b" % re.escape(col_lower), first_args_part, re.IGNORECASE):
                code = convert_key_to_code(key_part)
                if code:
                    return code
            elif orig_col and re.search(r"\b%s\b" % re.escape(orig_col), first_args_part, re.IGNORECASE):
                code = convert_key_to_code(key_part)
                if code:
                    return code
    return ""
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Query Analyzer Script (v12_gm - compare_col 키분리 보완)")
                ...
                for l_num, matched_cols in line_to_matched_cols.items():
                    ...
                    if match_type and matched_pair:
                        ...
                        if len(pair_in_order) >= 2:
                            ...
                            # [2026-08-11 v12 보완]
                            # 라인 전체의 default.encrypt/decrypt 키를 무차별 공통 적용하지 않고,
                            # c1 및 c2 각 컬럼 자신에게 직접 적용된 키 인자만 독립적으로 탐색하여 보완한다.
                            if conv_k1 == "99":
                                line_k1 = extract_line_enc_key_for_col(c1, orig_c1, info["matched_line"])
                                if line_k1:
                                    conv_k1 = line_k1

                            if conv_k2 == "99":
                                line_k2 = extract_line_enc_key_for_col(c2, orig_c2, info["matched_line"])
                                if line_k2:
                                    conv_k2 = line_k2
