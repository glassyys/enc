...
# -*- coding: utf-8 -*-
# ===============================================================
# p190872_local_chk_v07_gm.py(20260624)
#
# [소스 내용 정리]
...
#
# [실행 형식]
# # python p190872_local_chk_v07_gm.py <검색기준테이블> <검색디렉토리> <검색결과테이블명> [--mid <MID목록>] [--db] [--conf <설정파일>] [--where <old|new>] [--chk <default|encdec_no>]
#
# [실행 예시]
# # 1. 기본 실행 예시 (MID 전체 검색, DB 미적재):
# # python p190872_local_chk_v07_gm.py my_db.my_ref_table D:\chksrc\sources my_db.my_result_table
#
# # 2. 특정 MID(subdirectories) 검색 및 DB 적재:
# # python p190872_local_chk_v07_gm.py my_db.my_ref_table D:\chksrc\sources my_db.my_result_table --mid aaa,bbb,ccc --db --conf D:\chksrc\mysql.conf
#
# [수정 이력]
# ─────────────────────────────────────────────────────────────
# v05_gm (2026-06-16)
...
# 결과 파일 / 테이블 최종 필드 레이아웃 (mid 필드 추가)
CSV_FIELDNAMES = [
...
# ============================================================
def main():
    parser = argparse.ArgumentParser(description="Query Analyzer Script (v07_gm)")
    parser.add_argument("ref_table", help="검색기준테이블")
    parser.add_argument("search_dir", help="검색디렉토리")
    parser.add_argument("out_table", help="검색결과테이블명")  # 9차 수정 파라미터 추가
    ...
    for mid, files in source_files_by_mid.items():
        ...
        if included_results:
            ...
            if args.db:
                ...
                db_load_table(mysql_conf, fq_out_table, _DDL_CREATE_RESULT, _SQL_INSERT_RESULT, batch, mid, "결과데이터")
        else:
