...
# ============================================================
# 핵심 매칭 로직
# 조건1: 쿼리의 소스 또는 타겟 테이블 중 ref_row["tbl_name"] 과 일치
# 조건2: cols 파싱 결과의 각 칼럼명이 쿼리 텍스트에 \b완전일치로 포함
#         → 칼럼명별로 순환, 각각 line_number / matched_line 추출
# ============================================================
def build_match_rows(
    ...
    """
    ref_rows 각 행에 대해:
      1) tbl_name 이 소스/타겟 테이블에 존재하는지 확인
      2) cols 파싱 결과의 칼럼명별로 쿼리 내 \b완전일치 검색
      3) 매칭 시 원본 행 전체 칼럼값 + 매칭 결과 칼럼값으로 결과 생성
    """
    results   = []
    ...
    for ref_row in ref_rows:
        ...
        if not col_items:
            # cols 가 비어있어도 tbl_name 매칭 자체를 기록 (칼럼명 공란)
        ...
        for col_item in col_items:
            ...
            if rx is None:
                try:
                    rx = re.compile(r"\b%s\b" % re.escape(col_name), re.IGNORECASE)
...
def main():
    ...
    print("  - 매칭 건수        : %8d 건" % len(result_buffer))
    print("-" * 70)

    if not result_buffer:
        print("[WARN] 매칭된 결과가 없습니다. CSV 및 DB 저장을 건너뜁니다.")
        print("=" * 70)
        print(" 로컬 소스 tbl_name + cols 칼럼 매칭 탐색 완료 (매칭 없음)")
        print("=" * 70)
        sys.exit(0)
