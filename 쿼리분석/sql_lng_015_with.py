...
## python3 /home/p190872/chksrc/sql_lng_015_with.py /NAS/MIDP/DBMSVC/MIDP/TMT --mode DETAIL --db
## python3 /home/p190872/chksrc/sql_lng_015_with.py /NAS/MIDP/DBMSVC/MIDP/TDIA --mode SIMPLE --db
## python3 /home/p190872/chksrc/sql_lng_015_with.py /NAS/MIDP/DBMSVC/MIDP/TDIA --mode DETAIL --db
## python3 /home/p190872/chksrc/sql_lng_015_with.py /NAS/MIDP/DBMSVC/MIDP/TDM --mode SIMPLE --db
## python3 /home/p190872/chksrc/sql_lng_015_with.py /NAS/MIDP/DBMSVC/MIDP/TDM --mode DETAIL --db
## python3 /home/p190872/chksrc/sql_lng_015_with.py /NAS/MIDP/DBMSVC/MIDP/SID --mode SIMPLE --db
## python3 /home/p190872/chksrc/sql_lng_015_with.py /NAS/MIDP/DBMSVC/MIDP/SID --mode DETAIL --db
## 
...
## ■생성되는 테이블 예시
## 실행:
## python3 sql_lng_015_with.py /data/project1 --mode DETAIL --db
## 
## 생성 테이블명:
## sql_lng_015_with_project1_detail
## 아래 현재소스에서
...
## 파일은 sql_lng_013_with.py 로 생성요청
##
## 테이블생성전 DROP 명령추가(sql_lng_015_with.py)

#!/usr/bin/env python3
# sql_lng_015_with.py
#
# 실행방법:
# python3 sql_lng_015_with.py 소스디렉토리 [--mode SIMPLE|DETAIL] [--db] [--conf 경로]
#
# 옵션:
...
def _mysql_connect(conf):
    ...
    if _MYSQL_DRIVER == 'connector':
        ...
        raise ImportError("MySQL 드라이버가 없습니다.")
...
def db_insert_all(rows_buffer, run_id, mysql_conf, source_dir, mode):
    ...
    try:
        ...
        # 테이블이 이미 존재하면 삭제 (컬럼 구조 변경 등에 대응)
        ...
        cursor.close()
        conn.close()
        print(f"MySQL 테이블 DROP 후 재생성/적재 완료: {table_name} ({inserted}건)")
...
def parse_args():
    ...
    while i < len(args):
        if args[i] == "--mode":
            ...
            i += 1
    if src_dir is None:
        print("사용법: python3 sql_lng_015_with.py 소스디렉토리 [--mode SIMPLE|DETAIL] [--db] [--conf 경로]")
        sys.exit(1)
...
def has_top_level_dml(query, dml_keyword):
    ...
    return False
# ============================================================
# detect_real_sql_type() 에 DROP 명시적으로 처리 추가 (안전장치)
# ============================================================
...
def extract_target_tables(query):
    ...
    patterns = [
        ...
        # UPDATE, DELETE, MERGE, ALTER, TRUNCATE (기존)
        r'(?<![_\w])\bUPDATE\s+([^\s(]+)',
        ...
        # ★ DROP TABLE 강화 (IF EXISTS 포함) ★
        r'\bDROP\s+TABLE\s+(?:IF\s+EXISTS\s+)?([^\s;]+)',
        r'\bDROP\s+VIEW\s+(?:IF\s+EXISTS\s+)?([^\s;]+)',   # VIEW도 혹시 모르니 추가
    ]
...
def compute_cte_depths(cte_map):
    ...
    def get_depth(cte_name, visiting=None):
        ...
        if not cte_deps:
            ...
            depth_map[cte_name] = max(get_depth(d, visiting.copy()) for d in cte_deps) + 1
        return depth_map[cte_name]
