...
# v003_local (2026-06-30)
#   [패킷 꼬임 수정] Packet sequence number wrong (got 1 expected 2) 오류 해결
#   - _get_cursor() 제거: 쿼리마다 새 커서를 발급·사용·즉시 닫는 _exec() / _fetchall() 헬퍼 도입
#   - load_search_pairs_from_db(): SHOW TABLES / SHOW COLUMNS / SELECT 각각 별도 커서로 분리
#   - SHOW TABLES LIKE 절의 % 포맷팅 제거 → execute(sql, (ref_table,)) 파라미터 바인딩으로 변경
#   - db_insert_col_match_all() / db_insert_sql_all(): 매 청크마다 새 커서 사용 후 즉시 닫기
#   - connector 드라이버: 모든 커서 buffered=True 강제 적용
#   - pymysql 드라이버: 각 커서 close() 후 다음 커서를 열도록 순서 보장
#   [--where 옵션 추가] 검색 기준 테이블 조회 WHERE 조건 선택 기능
#   - --where 옵션 미지정 : WHERE tobe_enc_key <> ''
#   - --where OLD         : WHERE tobe_enc_key <> '' AND asis_enc_yn = 'Y'
#   - --where NEW         : WHERE tobe_enc_key <> '' AND asis_enc_yn = 'N'
#   - asis_enc_yn 칼럼 미존재 시 OLD/NEW 옵션 사용 오류 메시지 출력 후 종료
#   - 시작 배너 및 최종 요약에 '검색 WHERE 조건' 항목 추가
# ===============================================================
...
# ============================================================
# [v003] 커서 헬퍼 - 쿼리마다 새 커서를 열고 즉시 닫아
#         잔여 결과셋으로 인한 패킷 꼬임(Packet sequence number wrong)을 방지
# ============================================================
def _new_cursor(conn):
    """드라이버에 맞는 새 커서를 반환한다. connector → buffered=True 강제."""
    if _MYSQL_DRIVER == "connector":
        return conn.cursor(buffered=True)   # 결과셋을 즉시 클라이언트로 읽어옴
    return conn.cursor()                    # pymysql: 기본 커서


def _exec_query(conn, sql: str, params=None) -> list:
    """
    새 커서를 열어 sql 을 실행하고 fetchall() 결과를 반환한 뒤 커서를 닫는다.
    DDL/DML(반환 행 없음)에도 사용 가능 → 빈 리스트 반환.

    핵심 원칙
    ---------
    * 커서를 재사용하지 않는다 – 이전 결과셋이 버퍼에 남아 패킷 순번이 틀리는
      'Packet sequence number wrong' 오류를 원천 차단한다.
    * pymysql 에서 cursor.close() 를 호출하면 서버 측 잔여 패킷을 소비(drain)한다.
    """
    cursor = _new_cursor(conn)
    try:
...
def _exec_many(conn, sql: str, data: list) -> None:
    """
    새 커서를 열어 executemany() 를 실행한 뒤 커서를 닫는다.
    """
    cursor = _new_cursor(conn)
    try:
...
#
# [v003 수정]
#   - SHOW TABLES / SHOW COLUMNS / SELECT 를 각각 별도 커서(_exec_query)로 실행
#   - SHOW TABLES LIKE 의 % 를 문자열 포맷팅 대신 파라미터 바인딩으로 변경
#     (포맷팅 방식은 pymysql 이 파라미터 이스케이프를 오작동할 수 있음)
# ============================================================
def load_search_pairs_from_db(mysql_conf: dict, ref_table: str,
                              where_opt: str = None) -> tuple:
    """
    where_opt 값에 따라 WHERE 조건을 달리하여 검색 기준 쌍을 조회한다.

      None  → WHERE tobe_enc_key <> ''
      'OLD' → WHERE tobe_enc_key <> '' AND asis_enc_yn = 'Y'
      'NEW' → WHERE tobe_enc_key <> '' AND asis_enc_yn = 'N'
    """
    pairs = []
    seen  = set()
    conn  = None
    try:
        ...
        # ── ① 테이블 존재 여부 확인 ──────────────────────────────────
        # SHOW TABLES LIKE %s : % 를 포맷팅이 아닌 파라미터 바인딩으로 전달
        bare_table = ref_table.split(".")[-1]
        rows_exist = _exec_query(conn, "SHOW TABLES LIKE %s", (bare_table,))
        if not rows_exist:
            return [], "테이블이 존재하지 않습니다: %s" % ref_table

        # ── ② 칼럼 목록 조회 ─────────────────────────────────────────
        rows_cols = _exec_query(conn, "SHOW COLUMNS FROM `%s`" % ref_table)
        existing_cols = {row[0].lower() for row in rows_cols}

        # ── ③ WHERE 절 결정 ──────────────────────────────────────────
        if where_opt == "OLD":
            where_clause = "WHERE `tobe_enc_key` <> '' AND `asis_enc_yn` = 'Y'"
        elif where_opt == "NEW":
            where_clause = "WHERE `tobe_enc_key` <> '' AND `asis_enc_yn` = 'N'"
        else:
            where_clause = "WHERE `tobe_enc_key` <> ''"

        # asis_enc_yn 칼럼이 실제로 존재하지 않을 경우 OLD/NEW 조건 사용 불가
        if where_opt in ("OLD", "NEW") and "asis_enc_yn" not in existing_cols:
            return [], (
                "테이블 `%s` 에 `asis_enc_yn` 칼럼이 없습니다. "
                "--where %s 옵션을 사용하려면 해당 칼럼이 필요합니다."
                % (ref_table, where_opt)
            )
        ...
        sql  = "SELECT %s FROM `%s` %s" % (
            ", ".join(select_cols), ref_table, where_clause
        )
        rows = _exec_query(conn, sql)
...
#
# [v003 수정]
#   - DDL(DROP/CREATE) 과 DML(INSERT) 을 각각 _exec_query / _exec_many 로 분리
#   - 매 청크 executemany 를 _exec_many 내부에서 새 커서로 실행 후 즉시 닫음
#   - conn 은 한 번만 열고 마지막에 닫음 (커서만 교체)
# ============================================================
def db_insert_col_match_all(col_match_buffer: list, run_id: str, op_dtm: str,
    ...
    try:
        ...
        # ── DROP ────────────────────────────────────────────────────
        _exec_query(conn, _DDL_DROP_COL_MATCH.format(table=table_name))
        conn.commit()

        # ── CREATE ──────────────────────────────────────────────────
        _exec_query(conn, _DDL_CREATE_COL_MATCH.format(table=table_name))
        conn.commit()
        ...
        for i in range(0, len(batch), chunk_size):
            ...
            _exec_many(conn, _SQL_INSERT_COL_MATCH.format(table=table_name), chunk)
            conn.commit()
...
#
# [v003 수정] db_insert_col_match_all 과 동일한 패턴 적용
# ============================================================
def db_insert_sql_all(sql_buffer: list, run_id: str, op_dtm: str,
    ...
    try:
        ...
        _exec_query(conn, _DDL_DROP_SQL.format(table=table_name))
        conn.commit()

        _exec_query(conn, _DDL_CREATE_SQL.format(table=table_name))
        conn.commit()
        ...
        for i in range(0, len(batch), chunk_size):
            chunk = batch[i:i + chunk_size]
            _exec_many(conn, _SQL_INSERT_SQL.format(table=table_name), chunk)
            conn.commit()
...
def parse_args() -> tuple:
    ...
    while i < len(args):
        if args[i] == "--mode":
            ...
            if i + 1 < len(args):
                where_opt = args[i + 1].upper()
                if where_opt not in ("OLD", "NEW"):
                    print("[오류] --where 값은 OLD 또는 NEW 이어야 합니다.")
                    sys.exit(1)
                i += 2
            else:
    ...
    if src_dir is None or ref_table is None:
        print("사용법: python3 %s.py <검색대상_디렉토리> <검색기준테이블> "
              ...
              "[--where OLD|NEW]" % PROGRAM_NAME)
        print("")
        print("  --where 옵션 미지정 : tobe_enc_key <> '' 조건으로 조회")
        print("  --where OLD         : tobe_enc_key <> '' AND asis_enc_yn = 'Y' 조건으로 조회")
        print("  --where NEW         : tobe_enc_key <> '' AND asis_enc_yn = 'N' 조건으로 조회")
        print("")
        print("예시:")
        ...
        print("  python3 %s.py D:\\source enc_target_columns --mode SIMPLE --db --where OLD" % PROGRAM_NAME)
        print("  python3 %s.py /NAS/MIDP/SRC enc_target_columns --mode DETAIL --db "
              "--conf /home/user/mysql.conf --where NEW" % PROGRAM_NAME)
        sys.exit(1)
    ...
    return src_dir, ref_table, mode, use_db, conf_path, where_opt
...
# ============================================================
def main():
    src_dir, ref_table, mode, use_db, conf_path, where_opt = parse_args()
    op_dtm = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    run_id = datetime.now().strftime("%Y%m%d_%H%M%S")

    # --where 옵션 → 실제 적용되는 WHERE 절 문자열 (출력용)
    if where_opt == "OLD":
        where_desc = "OLD  (tobe_enc_key <> '' AND asis_enc_yn = 'Y')"
    elif where_opt == "NEW":
        where_desc = "NEW  (tobe_enc_key <> '' AND asis_enc_yn = 'N')"
    else:
        where_desc = "없음 (tobe_enc_key <> '')"
    ...
    print("  실행 모드          : %s" % mode)
    print("  검색 WHERE 조건    : %s" % where_desc)
    ...
    search_pairs, db_err = load_search_pairs_from_db(mysql_conf, ref_table, where_opt)
    if db_err:
    ...
    # in/ 디렉토리에 검색 기준 복사본 저장 (p190872 접두사 적용)
