...
# -*- coding: utf-8 -*-
# ===============================================================
# sql_v12_full_new_02_local_col.py
#
# [수정 사항 요약]
#   1. 파라미터 방식 변경:
#      - <검색기준테이블> 파라미터는 제거
#      - --col: 쉼표(,)로 구분된 검색 대상 컬럼 리스트 직접 입력
#      - --in: 컬럼 리스트 파일 경로 입력 (파일명만 기재 시 현재 디렉토리에서 검색)
#      - 위치 인자 구성: <검색디렉토리> <검색결과테이블명>
#   2. "--chk" 옵션 제거:
#      - --chk 옵션 및 이에 따른 분기 적재 로직 전면 삭제
#   3. 결과파일/테이블 대상 축소:
#      - "default DB 테이블", "encode_no DB 테이블", "exclude DB 테이블" 적재 제외
#      - 오직 <검색결과테이블명> (전체 매칭)과 "비교DB테이블"(diff_cols)만 생성 및 적재
#   - Python 2.7.5 호환성 전면 적용 및 기존 암복호화 비교 정합성 매칭 로직 보존
# ===============================================================
...
def get_source_files(search_dir, mids=None):
    result = {}
    search_dir = os.abspath(search_dir)
...
def build_db_batch(results, run_id, mid, op_dtm):
    return [
        (
            run_id,
            mid,
...
def main():
    ...
    parser.add_argument("--where", choices=["old", "new", "all"], help="검색기준테이블 조회 필터 (기존 호환용)", default=None)
    ...
    if not cols:
        print("[ERROR] 지정된 검색 대상 컬럼 리스트가 비어 있습니다.")
        sys.exit(1)
    ...
    for c in cols:
        c_lower = c.lower()
        if c_lower not in seen_cols_set:
            seen_cols_set.add(c_lower)
            unique_cols.append(c)

    print("[INFO] 총 %d 개의 고유 컬럼명을 수집하였습니다." % len(unique_cols))
    print("  수집된 컬럼 리스트: %s" % ", ".join(unique_cols))
    print("-" * 80)
    ...
    for mid, files in source_files_by_mid.items():
        ...
        ex_txt_path = os.path.abspath(os.path.join(out_dir, "p190872_%s_%s_exclude.txt" % (out_tbl_only, out_suffix)))
        diff_csv_path = os.path.abspath(os.path.join(out_dir, "p190872_%s_%s_diff_cols.csv" % (out_tbl_only, out_suffix)))
        ...
        if diff_cols_results:
            ...
            print("[INFO] '%s' MID에 대해 추출된 서로 다른 컬럼 비교 매칭 결과가 없습니다." % mid)

        # DB 적재 수행 (요청 3번: 결과 테이블과 diff_cols 비교 테이블만 적재)
        if args.db:
            if included_results:
        ...
        if args.db:
            ...
            summary_lines.append("     - 결과/비교 DB 테이블 : (적재 없음)")
        summary_lines.append("=" * 80)
