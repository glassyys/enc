...
# -*- coding: utf-8 -*-
# ===============================================================
# sql_v12_full_new_02_col.py
#
# 실행예시:
#   # SIMPLE 모드 (DB 미등록)
#   python3 /home/p190872/chksrc/sql_v12_full_new_02_col.py \
...
# 변경이력:
# ---------------------------------------------------------------
# v12_new_02_col (테이블 교차검증 제거 버전)
#   - 테이블 교차 검증 로직 제거: 테이블 매칭 여부를 고려하지 않고, 
#     주석이 제외된 query_text 상에서 해당 컬럼 단어(\b컬럼명\b)가 존재하면 추출
#   - 주석 내 컬럼 단어 오매칭 방지를 위해 main에서 주석 제거된 query를 query_text 인자로 전달
#   - 파일 절대 라인 매칭 탐색 및 fallback 시 주석 라인은 무시 처리
# ===============================================================
...
def build_col_match_rows(
    query_text: str, # 주석이 제외된 전처리 쿼리 텍스트
    sources: set,
    targets: set,
    crud_type: str,
    sql_type: str,
