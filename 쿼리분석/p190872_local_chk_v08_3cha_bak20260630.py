...
#   - 6차 수정요청 반영 (2026-06-27):
#     1) vscode_open_cmd 값 생성시 "[code]" 값 표시 제외
#     2) 결과 테이블 및 CSV 파일에 검색기준소스테이블의 source_file 칼럼 추가 적재 처리
#   - 7차 수정요청 반영 (2026-06-27):
#     타겟 테이블명을 <검색기준소스테이블명>_work 로 변경
# ===============================================================
...
def main():
            ...
            for col_lower, rx in compiled_col_patterns.items():
                if rx.search(clean_query_upper):
                    ...
                    for item in matched_lines_found:
                        ...
                        # 요청하신 포맷으로 추출 및 화면 출력 (상위 10개만 출력)
                        match_str = "[매칭] %s %s" % (vscode_cmd, orig_col_name)
                        content_str = "[내용] %s" % l_val.strip()
                        
                        if screen_match_print_count < 10:
                            print(match_str)
                            print(content_str)
                            print("-" * 80)
                            screen_match_print_count += 1
                        elif screen_match_print_count == 10:
                            print("[INFO] 상위 10개 매칭 결과만 화면에 출력되었습니다. 전체 결과는 파일 및 DB를 확인하세요.")
                            screen_match_print_count += 1
    ...
    out_table_name = "%s_work" % src_tbl_only
