...
# -*- coding: utf-8 -*-
# ===============================================================
# sql_v12_full_new_04_col.py
#
# 실행예시:
#   # SIMPLE 모드 (DB 미등록)
#   python /home/p190872/chksrc/sql_v12_full_new_04_col.py \
...
# ============================================================
# 타겟 테이블 추출 (INSERT ${var} TABLE ${aaa}.tb1 패턴 추가 보완)
# ============================================================
def extract_target_tables(query):
    targets  = set()
    patterns = [
        r"\bINSERT\s+(?:(?!SELECT|VALUES|FROM)[^\s(;]+\s+)*?TABLE\s+([^\s(]+)",
        r"\bINSERT\s+OVERWRITE\s+(?!TABLE\b)([^\s(]+)",
        r"\bINSERT\s+INTO\s+(?!TABLE\b)([^\s(]+)",
        r"\bINSERT\s+(?:\$\{[^}]+\}|\$\w+)\s+(?!TABLE\b)([^\s(]+)",
