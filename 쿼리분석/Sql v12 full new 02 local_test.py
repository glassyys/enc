...
# ============================================================
# 동적 테이블명 생성 (검색기준테이블 스키마 연동 처리 추가)
# ============================================================
def build_col_match_table_name(source_dir: str, mode: str, ref_table: str) -> str:
    last_dir = os.path.basename(os.path.normpath(source_dir))
    table_suffix = "%s_%s_%s_col_match" % (PROGRAM_NAME, last_dir, mode.lower())

    # <검색기준테이블>의 스키마가 입력되었는지 파싱 후 결합
    if "." in ref_table:
        parts = ref_table.split(".")
        schema_part = parts[0].replace("`", "")
        return "`%s`.`%s`" % (schema_part, table_suffix)
    else:
        return "`%s`" % table_suffix
...
def db_insert_col_match_all(col_match_buffer: list, run_id: str, op_dtm: str,
                            mysql_conf: dict, source_dir: str, mode: str, ref_table: str) -> tuple:
    table_name = build_col_match_table_name(source_dir, mode, ref_table)
    conn   = None
    cursor = None
    try:
        conn   = _mysql_connect(mysql_conf)
...
def main():
    ...
    # ── 서버 MySQL 테이블 조회 (파라미터 입력 전체 명칭 기준 처리 완료) ──────────────────
    ...
    # 🌟 요청사항 4번: 검색리스트 화면출력부분 생략 처리 완료
    print("-" * 70)

    # in/ 디렉토리에 검색 기준 복사본 저장 (일자_시간 명칭 제외 적용)
    ...
    # ── DB 적재 (--db 옵션) (요청사항 3번 반영됨) ──────────────────────────
    db_inserted = 0
    db_err_msg  = None
    db_table    = build_col_match_table_name(src_dir, mode, ref_table)

    if use_db:
        print("[INFO] MySQL 테이블 적재 시작: %s ..." % db_table)
        db_inserted, db_err_msg = db_insert_col_match_all(
            col_match_buffer, run_id, op_dtm, mysql_conf, src_dir, mode, ref_table
        )
        if db_err_msg:
