...
6. 소스 분석/비교 도구 전용 규칙 (p190872_local_chk_vXX 계열 프로그램 대상)
※ 아래 규칙은 전체 파이썬 소스 코드 생성/수정에 공통 적용되는 룰이 아니며, 오직 p190872_local_chk 계열 프로그램과 그 후속 업그레이드 버전에만 국한하여 적용합니다.

① 비교 CSV 파일 생성 대상 범위 (14차 수정요청 반영)
- "비교 CSV 파일" 생성 시 대상은 "default 분리 CSV"된 파일을 포함한 "결과 CSV 파일"에 포함된 전체 대상(default.encrypt 및 default.decrypt 단어가 들어간 라인 포함)으로 검색하여 추출합니다.
- 따라서 default.encrypt 와 default.decrypt 단어가 들어가 있는 라인도 포함해서 해당 "비교 CSV 파일" 추출 조건에 맞으면 추출하도록 합니다.

② default 암복호화 비교 구문 추출 강화 (15차 수정요청 반영)
- `default.decrypt` 및 `default.encrypt` 가 적용된 라인이라도, 이 껍데기를 인자 컬럼으로 정규화(치환)한 뒤 기존 비교 규칙(AS, =, CASE, 기타연산자 등)을 실행하여 모두 누락 없이 "비교 CSV 파일"에 추출하도록 합니다.
- 또한 `default.decrypt(col1) = column_name_not_col` 과 같이 비교 대상 컬럼이 기준 테이블에 등록되지 않은 컬럼이더라도, 리터럴(상수, NULL 등)이 아닌 테이블 컬럼끼리의 비교인 경우 예외적으로 비교 대상 쌍으로 수집하여 "비교 CSV 파일"에 추출합니다.

7. 공통 작업 및 백업 규칙
① 소스 수정 시 백업본 보관 원칙 (공통 적용)
- 본 워크스페이스 내의 모든 소스 수정 시에는 반드시 수정하기 전 원본 파일을 백업본으로 복사해 둔 후 수정을 진행합니다. (예: 파일명 뒤에 `_bakYYYYMMDD.py` 또는 `_bak15.py` 등의 버전을 덧붙여 생성)
